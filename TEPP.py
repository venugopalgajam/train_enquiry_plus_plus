    ...
    for tpl in list1:
        ...
        for offset in range(3):
            ...
            if week_day_2 not in second_list:
                second_list[week_day_2] = trains_btw_with_times(intr,dst,int(week_day_2))
...
def get_paths(src, dst, jdate,OnlyDirect=True):
    #print('get_paths',src,dst,jdate)# pylint: disable=E1601
    week_day = jdate.weekday()
    result = dict()
    
    stns_list,direct_trains = inter_stns(src, dst, week_day)
    res = list()
    ...
    for intr in stns_list:
        ...
        train_pairs = get_trainspair(src,intr[0],dst,week_day)
